...
export function RequirementsManager({ requirements }: { requirements: Requirement[] }) {
  ...
  async function create() {
    ...
    setSaving(true);
    try {
      await apiClient("/api/players/me/requirements", {
        method: "POST",
        body: JSON.stringify({
          title: title.trim(),
          description: description || null,
          totalAmountPaise: breakdown.reduce((s, b) => s + b.amountPaise, 0),
          breakdown,
        }),
      });
      setTitle("");
      ...
      toast.error(err instanceof Error ? err.message : "Could not create requirement");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function close(id: string) {
    try {
      await apiClient(`/api/players/me/requirements/${id}`, {
        method: "PUT",
      ...
      toast.error(err instanceof Error ? err.message : "Could not close requirement");
    }
  }
